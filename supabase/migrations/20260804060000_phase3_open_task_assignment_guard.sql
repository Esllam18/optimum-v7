...
begin
  ...
  if (p_open_unassigned or cardinality(p_assignee_role_ids)>0 or exists(select 1 from unnest(coalesce(p_assignee_user_ids,array[]::uuid[])) u where u<>auth.uid())) and not app_private.has_company_permission(p_company_id,'tasks.assign') then raise exception 'Permission denied'; end if;
