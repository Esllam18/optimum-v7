...
create or replace function app_private.user_has_company_permission(
  p_user_id uuid,
  p_company_id uuid,
  ...
  select
    ...
    or exists(
      ...
      where m.company_id = p_company_id
        ...
        and (
          r.slug = 'owner'
          or coalesce(mpo.allowed, rp.allowed, false)
        )
    );
$$;
...
revoke all on function app_private.user_has_company_permission(uuid, uuid, text) from public, anon, authenticated;
