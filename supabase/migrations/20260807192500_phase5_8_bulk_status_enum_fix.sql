-- Phase 5.8 Organization OS: make legacy bulk status helpers type-safe against membership_status.
...
begin
  ...
  begin
    target_status:=p_status::public.membership_status;
...
begin
  ...
  loop
    ...
    if item ? 'role_id' and nullif(item->>'role_id','') is not null then
      perform public.set_member_role(membership_id,(item->>'role_id')::uuid);
    end if;
    if item ? 'status' and nullif(item->>'status','') is not null then
      begin
        restored_status := (item->>'status')::public.membership_status;
      exception when invalid_text_representation then
        raise exception 'Invalid membership status';
      end;
