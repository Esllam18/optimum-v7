    ...
    {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={11} /> : <>
      <div className="v7-detail-section">
        <div className="v7-detail-grid">
          ...
          <div className="v7-detail-field"><span>{locale === 'ar' ? 'الجاهزية الكلية' : 'Overall readiness'}</span><strong>{progress.overall_percent ?? 0}%</strong></div>
          <div className="v7-detail-field"><span>{locale === 'ar' ? 'الأدلة المطلوبة' : 'Required evidence'}</span><strong>{progress.required_satisfied ?? 0}/{progress.required_total ?? 0}</strong></div>
...
export default function DeliveryPage({ workspace, locale }) {
  ...
  return <>
    ...
    <section className="v7-delivery-pulse"><div><span className="v7-eyebrow">{locale === 'ar' ? 'مسار التقديم' : 'SUBMISSION PIPELINE'}</span><strong>{locale === 'ar' ? 'من التجميع إلى التقديم بدون كسر مرجعية الـCDE' : 'From collection to submission without breaking CDE traceability'}</strong><small>{locale === 'ar' ? 'الحزم تظل مرتبطة بالمستندات الأصلية، والتجميد يثبت الإصدارات المقدمة فقط.' : 'Packages stay linked to canonical documents; freezing pins only the submitted versions.'}</small></div><div className="v7-delivery-pulse-metrics"><span><small>{locale === 'ar' ? 'الحزم' : 'Packages'}</small><strong>{directory ? (counts.packages ?? total) : '—'}</strong></span><span><small>{locale === 'ar' ? 'قيد التجميع' : 'Collecting'}</small><strong>{directory ? (counts.collecting ?? 0) : '—'}</strong></span><span><small>{locale === 'ar' ? 'مقدمة' : 'Submitted'}</small><strong>{directory ? ((counts.submitted ?? 0) + (counts.approved ?? 0)) : '—'}</strong></span><span><small>{locale === 'ar' ? 'الكابينات' : 'Cabinets'}</small><strong>{directory ? (counts.cabinets ?? 0) : '—'}</strong></span></div></section>
    <Panel title={locale === 'ar' ? 'حزم التقديم' : 'Submission packages'} description={locale === 'ar' ? 'بحث وتصفية Server-side داخل سياق المشروع أو الموقع الحالي.' : 'Server-side search and filtering inside the active project or site context.'}>
      <div className="v7-delivery-toolbar"><div className="v7-search-field"><Icon name="search" size={16} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder={locale === 'ar' ? 'ابحث برقم الحزمة أو العنوان…' : 'Search package number or title…'} /></div><select value={status} onChange={e => setStatus(e.target.value)}><option value="">{locale === 'ar' ? 'كل الحالات' : 'All statuses'}</option><option value="collecting">{locale === 'ar' ? 'قيد التجميع' : 'Collecting'}</option><option value="ready">{locale === 'ar' ? 'جاهز' : 'Ready'}</option><option value="submitted">{locale === 'ar' ? 'مقدم' : 'Submitted'}</option><option value="approved">{locale === 'ar' ? 'معتمد' : 'Approved'}</option><option value="rejected">{locale === 'ar' ? 'مرفوض' : 'Rejected'}</option><option value="archived">{locale === 'ar' ? 'مؤرشف' : 'Archived'}</option></select><span className="v7-toolbar-count">{total} {locale === 'ar' ? 'حزمة' : 'packages'}</span></div>
