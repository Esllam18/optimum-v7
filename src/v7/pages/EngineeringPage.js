    ...
    {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={11} /> : <>
      <div className="v7-detail-section">
        <div className="v7-detail-grid">
          ...
          <div className="v7-detail-field"><span>{locale === 'ar' ? 'المراجعة الحالية' : 'Current revision'}</span><strong>{current?.revision_code || '—'}</strong></div>
          <div className="v7-detail-field"><span>{locale === 'ar' ? 'ملاحظات المراجعة المفتوحة' : 'Open review marks'}</span><strong>{openMarks.length}</strong></div>
          <div className="v7-detail-field"><span>{locale === 'ar' ? 'بنود الحصر' : 'BOQ lines'}</span><strong>{normalizeArray(data.boq).length}</strong></div>
        </div>
      ...
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'سجل المراجعات' : 'Revision history'}</h3>{normalizeArray(data.revisions).length ? <div className="v7-linked-list">{normalizeArray(data.revisions).map(revision => <article key={revision.id}><Icon name="layers" size={15} /><span><strong>{revision.revision_code} · {revision.status}</strong><small>{formatDate(revision.published_at || revision.submitted_at || revision.updated_at, locale)}{revision.change_note ? ` · ${revision.change_note}` : ''}</small></span>{revision.id === drawing.current_revision_id ? <Badge tone="success">{locale === 'ar' ? 'الحالية' : 'Current'}</Badge> : <Badge tone={statusTone(revision.status)}>{revision.status}</Badge>}</article>)}</div> : <EmptyState icon="layers" title={tx(locale, 'noData')} />}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'ملاحظات المراجعة' : 'Review marks'}</h3>{normalizeArray(data.marks).length ? <div className="v7-linked-list">{normalizeArray(data.marks).slice(0, 20).map(mark => <article key={mark.id}><Icon name={mark.status === 'resolved' ? 'check' : 'alert'} size={15} /><span><strong>{mark.title || mark.body || (locale === 'ar' ? 'ملاحظة مراجعة' : 'Review mark')}</strong><small>{mark.priority || 'normal'} · {mark.status} · {formatDate(mark.created_at, locale)}</small></span><Badge tone={statusTone(mark.status)}>{mark.status}</Badge></article>)}</div> : <p>{locale === 'ar' ? 'لا توجد ملاحظات مراجعة مرتبطة بهذه الرسمة.' : 'No review marks are attached to this drawing.'}</p>}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'الحصر الحالي' : 'Current BOQ'}</h3>{normalizeArray(data.boq).length ? <div className="v7-mini-table"><div><strong>{locale === 'ar' ? 'البند' : 'Item'}</strong><strong>{locale === 'ar' ? 'الوصف' : 'Description'}</strong><strong>{locale === 'ar' ? 'الكمية' : 'Qty'}</strong></div>{normalizeArray(data.boq).slice(0, 30).map(item => <div key={item.id}><span>{item.item_code}</span><span>{locale === 'ar' ? item.description_ar : item.description_en}</span><span>{item.quantity} {item.unit}</span></div>)}</div> : <p>{locale === 'ar' ? 'لا توجد بنود حصر محفوظة للمراجعة الحالية.' : 'No BOQ rows are stored for the current revision.'}</p>}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'المستندات المرتبطة' : 'Linked documents'}</h3>{normalizeArray(data.links).length ? <div className="v7-linked-list">{normalizeArray(data.links).map(link => <button key={link.id} onClick={() => router.push(projectAreaHref('documents', context, { document: link.document_id }))}><Icon name="file" size={15} /><span><strong>{link.relation_type || (locale === 'ar' ? 'مستند مرجعي' : 'Reference document')}</strong><small>{link.document_id}</small></span><Icon name="chevron" size={14} /></button>)}</div> : <p>{locale === 'ar' ? 'لا توجد مستندات CDE أصلية مرتبطة بهذه الرسمة.' : 'No canonical CDE documents are linked to this drawing.'}</p>}</div>
    </>}
  </SideSheet>;
}
...
export default function EngineeringPage({ workspace, locale }) {
  ...
  return <>
    ...
    <div className="v7-engineering-hero"><div><span className="v7-eyebrow">{locale === 'ar' ? 'مساحة CAD · محرك V7 الأصلي' : 'V7 CAD WORKSPACE · NATIVE CORE'}</span><h2>{locale === 'ar' ? 'ارسم ← افهم ← احصر ← راجع ← أصدر' : 'Draw → understand → take off → review → issue'}</h2><p>{locale === 'ar' ? 'واجهة هندسية مستقلة في V7، مع محرك الرسم والحصر وDXF مفصول في وحدة Core قابلة للاختبار ومستقلة عن الواجهة القديمة.' : 'A native V7 engineering workspace with the drawing, takeoff and DXF engine extracted into a testable core module independent from the previous UI runtime.'}</p><div className="v7-inline-actions"><Badge tone="success">{locale === 'ar' ? 'محرك الرسم محفوظ' : 'Core parity preserved'}</Badge><Badge tone="brand">{locale === 'ar' ? 'مساحة V7 أصلية' : 'Native V7 workspace'}</Badge></div></div><div className="v7-engineering-graphic"><span><Icon name="drafting" size={36} /></span><i /><span><Icon name="layers" size={32} /></span><i /><span><Icon name="check" size={32} /></span></div></div>
