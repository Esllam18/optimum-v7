...
export default function WorkIntelligencePage({ workspace, locale }) {
  ...
  useEffect(() => {
    ...
    else if (current === 'capacity') request = canWorkload ? api.rpc('work_capacity_plan',{p_company_id:company,p_from:new Date().toISOString().slice(0,10),p_days:14},{cacheTtlMs:15_000,signal:c.signal,dedupe:true}) : Promise.resolve({denied:true});
    ...
    return ()=>c.abort();
  },[workspace.company.id,current,reload,canWorkload]);

  const go = (view) => router.push(`/v7/work-intelligence?view=${view}`);
  ...
  return <>
    ...
    {!data ? <Skeleton lines={12} /> : current==='cockpit' ? <Cockpit data={data} locale={locale} router={router} /> : current==='capacity' ? <Capacity data={data} locale={locale} /> : current==='dependencies' ? <Dependencies data={data} locale={locale} router={router} /> : <Setup data={data} locale={locale} canSetup={canSetup} />}
  </>;
}
...
function Capacity({data,locale}){if(data.denied)return <EmptyState icon="shield" title={locale==='ar'?'تحتاج صلاحية عرض أحمال الفريق':'Workload permission required'}/>;const members=normalizeArray(data.members),cells=normalizeArray(data.cells);return <Panel title={locale==='ar'?'خطة القدرة · 14 يوم':'Capacity plan · 14 days'}><div className="v7-capacity-table"><header><span>{locale==='ar'?'العضو':'Member'}</span><span>{locale==='ar'?'السعة':'Capacity'}</span><span>{locale==='ar'?'المخطط':'Planned'}</span><span>{locale==='ar'?'الاستخدام':'Utilization'}</span></header>{members.map(m=>{const rows=cells.filter(c=>c.user_id===m.user_id);const cap=rows.reduce((s,c)=>s+Number(c.capacity_minutes||0),0),planned=rows.reduce((s,c)=>s+Number(c.planned_minutes||0),0),u=cap?Math.round(planned/cap*100):0;return <article key={m.user_id}><div><strong>{m.name}</strong><small>{m.job_title||m.department||'—'}</small></div><span>{Math.round(cap/60)}h</span><span>{Math.round(planned/60)}h</span><span className={u>100?'is-danger':u>85?'is-warning':''}><b>{u}%</b><i><em style={{width:`${Math.min(100,u)}%`}}/></i></span></article>})}</div></Panel>}
