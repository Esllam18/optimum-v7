...
function MemberDetail({ membershipId, locale, onClose }) {
  ...
  return <SideSheet open={Boolean(membershipId)} onClose={onClose} eyebrow="MEMBER 360" title={data ? profile.full_name || tx(locale, 'members') : tx(locale, 'loading')} subtitle={data ? [member.job_title, member.department, locale === 'ar' ? role.name_ar : role.name_en].filter(Boolean).join(' · ') : ''}>
    {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={10} /> : <>
      ...
      <div className="v7-detail-section"><div className="v7-detail-grid"><div className="v7-detail-field"><span>Role</span><strong>{locale === 'ar' ? role.name_ar : role.name_en}</strong></div><div className="v7-detail-field"><span>Work mode</span><strong>{member.work_mode || '—'}</strong></div><div className="v7-detail-field"><span>Joined</span><strong>{formatDate(member.joined_at, locale)}</strong></div><div className="v7-detail-field"><span>Capacity</span><strong>{member.weekly_capacity_hours ?? '—'} h/w</strong></div></div></div>
      <div className="v7-detail-section"><h3>Access window</h3><div className="v7-detail-grid"><div className="v7-detail-field"><span>Starts</span><strong>{formatDate(member.access_starts_at, locale)}</strong></div><div className="v7-detail-field"><span>Ends</span><strong>{formatDate(member.access_ends_at, locale)}</strong></div><div className="v7-detail-field"><span>Lifecycle</span><strong>{member.lifecycle_stage || 'active'}</strong></div><div className="v7-detail-field"><span>Primary site</span><strong>{member.primary_site_id ? 'Assigned' : 'Company scope'}</strong></div></div></div>
      <div className="v7-detail-section"><h3>Organization units</h3>{normalizeArray(data.organization_units).length ? <div className="v7-linked-list">{normalizeArray(data.organization_units).map(unit => <article key={unit.id || unit.unit_id}><Icon name="users" size={15} /><span><strong>{locale === 'ar' ? unit.name_ar : unit.name_en}</strong><small>{unit.unit_type || unit.title || 'Organization unit'}</small></span></article>)}</div> : <p>No organization unit is explicitly assigned.</p>}</div>
      <div className="v7-detail-section"><h3>Effective permissions</h3><div className="v7-permission-cloud">{permissions.map(key => <Badge key={key} tone="neutral">{key}</Badge>)}</div></div>
      <div className="v7-detail-section"><h3>Scope rules</h3>{normalizeArray(data.scope_rules).length ? <div className="v7-linked-list">{normalizeArray(data.scope_rules).map(rule => <article key={rule.id}><Icon name="shield" size={15} /><span><strong>{rule.effect} · {rule.scope_type}</strong><small>{rule.permission_key || 'All effective permissions'} · {rule.target_id || 'Company'}</small></span></article>)}</div> : <p>No explicit scope restriction is applied; effective company access rules are used.</p>}</div>
    </>}
  </SideSheet>;
}
  ...
  return <>
    ...
    <Panel>
      <div className="v7-people-toolbar">
        ...
        <select value={status} onChange={e => { setStatus(e.target.value); setOffset(0); }}><option value="">{locale === 'ar' ? 'كل الحالات' : 'All statuses'}</option><option value="active">Active</option><option value="invited">Invited</option><option value="suspended">Suspended</option><option value="left">Left</option></select>
