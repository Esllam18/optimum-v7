      ...
      {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={10} /> : <>
        ...
        <div className="v7-detail-section">
          <div className="v7-detail-grid">
            ...
            <div className="v7-detail-field"><span>{locale === 'ar' ? 'الموقع داخل CDE' : 'CDE location'}</span><strong>{[data.site?.name, data.cabinet?.name, data.folder?.name].filter(Boolean).join(' · ') || 'Project CDE'}</strong></div>
          </div>
