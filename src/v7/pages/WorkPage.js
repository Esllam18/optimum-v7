...
function TaskDetail({ taskId, locale, onClose, router }) {
  ...
  return <SideSheet open={Boolean(taskId)} onClose={onClose} eyebrow="WORK 360" title={data ? `#${task.task_number || '—'} · ${task.title || tx(locale, 'task')}` : tx(locale, 'loading')} subtitle={data ? `${task.task_type || 'task'} · ${task.priority || 'medium'}` : ''}>
    {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={9} /> : <>
      ...
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'قائمة التحقق' : 'Checklist'}</h3>{normalizeArray(data.checklist).length ? <div className="v7-linked-list">{normalizeArray(data.checklist).map(item => <article key={item.id}><Icon name={item.is_completed ? 'check' : 'clock'} size={15} /><span><strong>{item.body}</strong><small>{item.is_completed ? (locale === 'ar' ? 'مكتمل' : 'Completed') : (locale === 'ar' ? 'قيد التنفيذ' : 'Pending')}</small></span></article>)}</div> : <EmptyState icon="check" title={tx(locale, 'noData')} />}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'الأشخاص' : 'People'}</h3>{normalizeArray(data.assignments).length ? <div className="v7-linked-list">{normalizeArray(data.assignments).map(item => <article key={item.id}><Icon name="users" size={15} /><span><strong>{item.name || item.role_name_en || item.role_name_ar || 'Assignee'}</strong><small>{item.role_name_en || item.role_name_ar || 'Direct assignment'}</small></span></article>)}</div> : <p>{locale === 'ar' ? 'لا توجد تعيينات مباشرة؛ قد تكون المهمة مملوكة لمستخدم أو متاحة للاستلام.' : 'No explicit assignments. The task may be owned or open for claiming.'}</p>}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'الاعتماديات' : 'Dependencies'}</h3>{normalizeArray(data.blockers).length ? <div className="v7-linked-list">{normalizeArray(data.blockers).map(item => <button key={item.id} onClick={() => router.push(projectAreaHref('work', context, { task: item.id }))}><Icon name="alert" size={15} /><span><strong>#{item.task_number} · {item.title}</strong><small>{item.status}</small></span><Icon name="chevron" size={14} /></button>)}</div> : <p>{locale === 'ar' ? 'لا توجد معطلات غير مكتملة ضمن نطاق رؤيتك.' : 'No unfinished visible blockers.'}</p>}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'النشاط' : 'Activity'}</h3>{normalizeArray(data.events).length ? <div className="v7-linked-list">{normalizeArray(data.events).slice(0, 12).map(item => <article key={item.id}><Icon name="activity" size={15} /><span><strong>{item.event_type}</strong><small>{item.actor_name || 'System'} · {formatDate(item.created_at, locale)}</small></span></article>)}</div> : <p>{locale === 'ar' ? 'لم يتم تسجيل نشاط بعد.' : 'No activity has been recorded yet.'}</p>}</div>
      <div className="v7-detail-section"><h3>{locale === 'ar' ? 'المحادثة' : 'Conversation'}</h3>{normalizeArray(data.comments).length ? <div className="v7-linked-list">{normalizeArray(data.comments).slice(-12).reverse().map(item => <article key={item.id}><Icon name="file" size={15} /><span><strong>{item.author_name || 'Member'}</strong><small>{item.body}</small></span></article>)}</div> : <p>{locale === 'ar' ? 'لا توجد تعليقات بعد.' : 'No comments yet.'}</p>}</div>
    </>}
  </SideSheet>;
}
