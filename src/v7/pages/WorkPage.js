...
function TaskDetail({ taskId, locale, onClose, router }) {
  ...
  useEffect(() => {
    ...
    api.rpc('work_task_detail', { p_task_id: taskId }, { cacheTtlMs: 8_000, signal: c.signal, dedupe: true })
    ...
    {error ? <ErrorState title={tx(locale, 'networkIssue')} description={error.message} /> : !data ? <Skeleton lines={9} /> : <>
      <div className="v7-detail-section">
        <div className="v7-detail-grid">
          ...
          <div className="v7-detail-field"><span>Risk</span><strong>{data.risk?.level || 'low'} · {data.risk?.score ?? 0}</strong></div>
        ...
        <div className="v7-inline-actions">
          {task.document_id ? <Button icon="file" onClick={() => router.push(projectAreaHref('documents', context, { document: task.document_id }))}>Open document</Button> : null}
          {task.project_id ? <Button icon="briefcase" onClick={() => router.push(projectAreaHref('project', context))}>Project 360</Button> : null}
        </div>
      </div>
      <div className="v7-detail-section"><h3>Checklist</h3>{normalizeArray(data.checklist).length ? <div className="v7-linked-list">{normalizeArray(data.checklist).map(item => <article key={item.id}><Icon name={item.is_completed ? 'check' : 'clock'} size={15} /><span><strong>{item.body}</strong><small>{item.is_completed ? 'Completed' : 'Pending'}</small></span></article>)}</div> : <EmptyState icon="check" title={tx(locale, 'noData')} />}</div>
      <div className="v7-detail-section"><h3>People</h3>{normalizeArray(data.assignments).length ? <div className="v7-linked-list">{normalizeArray(data.assignments).map(item => <article key={item.id}><Icon name="users" size={15} /><span><strong>{item.name || item.role_name_en || item.role_name_ar || 'Assignee'}</strong><small>{item.role_name_en || item.role_name_ar || 'Direct assignment'}</small></span></article>)}</div> : <p>No explicit assignments. The task may be owned or open for claiming.</p>}</div>
      <div className="v7-detail-section"><h3>Dependencies</h3>{normalizeArray(data.blockers).length ? <div className="v7-linked-list">{normalizeArray(data.blockers).map(item => <button key={item.id} onClick={() => router.push(projectAreaHref('work', context, { task: item.id }))}><Icon name="alert" size={15} /><span><strong>#{item.task_number} · {item.title}</strong><small>{item.status}</small></span><Icon name="chevron" size={14} /></button>)}</div> : <p>No unfinished visible blockers.</p>}</div>
      <div className="v7-detail-section"><h3>Activity</h3>{normalizeArray(data.events).length ? <div className="v7-linked-list">{normalizeArray(data.events).slice(0, 12).map(item => <article key={item.id}><Icon name="activity" size={15} /><span><strong>{item.event_type}</strong><small>{item.actor_name || 'System'} · {formatDate(item.created_at, locale)}</small></span></article>)}</div> : <p>No activity has been recorded yet.</p>}</div>
      <div className="v7-detail-section"><h3>Conversation</h3>{normalizeArray(data.comments).length ? <div className="v7-linked-list">{normalizeArray(data.comments).slice(-12).reverse().map(item => <article key={item.id}><Icon name="file" size={15} /><span><strong>{item.author_name || 'Member'}</strong><small>{item.body}</small></span></article>)}</div> : <p>No comments yet.</p>}</div>
    </>}
  </SideSheet>;
}
...
function CreateTaskSheet({ open, onClose, workspace, locale, onCreated, context }) {
  ...
  return <SideSheet open={open} onClose={onClose} eyebrow="QUICK CREATE" title={locale === 'ar' ? 'مهمة جديدة' : 'New task'} subtitle={contextText || (locale === 'ar' ? 'تُسند إليك افتراضيًا ويمكن تعديلها بعد الإنشاء.' : 'Assigned to you by default; full controls remain available after creation.')}>
    <form className="v7-form-stack" onSubmit={submit}>
      ...
      <div className="v7-form-grid"><label><span>{locale === 'ar' ? 'الأولوية' : 'Priority'}</span><select value={form.priority} onChange={e => set('priority', e.target.value)}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="urgent">Urgent</option></select></label><label><span>{tx(locale, 'due')}</span><input type="datetime-local" value={form.due} onChange={e => set('due', e.target.value)} /></label></div>
...
export default function WorkPage({ workspace, locale }) {
  ...
  return <>
    <PageHeader eyebrow="WORK OS" title={tx(locale, 'workCenter')} description={tx(locale, 'workCenterSub')} actions={can(workspace, 'tasks.create') ? <Button variant="primary" icon="plus" onClick={() => setCreateOpen(true)}>{tx(locale, 'quickCreate')}</Button> : null}>
    ...
    {!scoped ? <section className="v7-work-pulse">
      <div className="v7-work-pulse-copy"><span className="v7-eyebrow">EXECUTION FOCUS</span><strong>{locale === 'ar' ? 'ما الذي يحتاج حركة الآن؟' : 'What needs movement now?'}</strong><small>{locale === 'ar' ? 'المتأخر والمحجوب والمستحق اليوم أمامك قبل بقية قائمة العمل.' : 'Overdue, blocked, and due-today work is surfaced before the rest of the queue.'}</small></div>
      <div className="v7-work-pulse-metrics">
        <button className={Number(metrics?.overdue || 0) ? 'is-danger' : ''} onClick={() => { setDueFilter('overdue'); setPage(0); }}><span>Overdue</span><strong>{metrics?.overdue ?? '—'}</strong></button>
        <button className={Number(metrics?.blocked || 0) ? 'is-warning' : ''} onClick={() => { setStatus('blocked'); setPage(0); }}><span>Blocked</span><strong>{metrics?.blocked ?? '—'}</strong></button>
        <button onClick={() => { setDueFilter('today'); setPage(0); }}><span>Due today</span><strong>{metrics?.due_today ?? '—'}</strong></button>
    ...
    <TaskDetail taskId={taskId} locale={locale} onClose={closeTask} router={router} />
