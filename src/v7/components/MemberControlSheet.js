...
export default function MemberControlSheet({ open, onClose, workspace, membershipId, locale, onSaved }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setPreferences(prefs[0] || null); setCompensation(comp[0] || null);
      setForm({
  ...
  const save = async event => {
    ...
    try {
      ...
      await api.rpc('save_member_control_profile', {
        ...
        p_compensation:{}, p_permission_overrides:[],
  ...
  return <SideSheet open={open} onClose={onClose} eyebrow={locale === 'ar' ? 'إدارة العضو 360°' : 'MEMBER CONTROL 360'} title={data?.profile?.full_name || (locale === 'ar' ? 'إدارة العضو' : 'Manage member')} subtitle={data ? `${roleName || '—'} · ${member.status || '—'}` : ''}>
    {error && !data ? <ErrorState title={locale === 'ar' ? 'تعذر تحميل ملف العضو' : 'Could not load member'} description={error} /> : !form ? <Skeleton lines={12} /> : <form className="v7-form-stack v7-member-control" onSubmit={save}>
      ...
      {compensation ? <section className="v7-detail-section v7-sensitive-panel"><h3><Icon name="lock" size={15} /> {locale === 'ar' ? 'التعويضات' : 'Compensation'}</h3><div className="v7-detail-grid"><div className="v7-detail-field"><span>{locale === 'ar' ? 'الراتب' : 'Salary'}</span><strong>{compensation.salary_amount ?? '—'} {compensation.currency || ''}</strong></div><div className="v7-detail-field"><span>{locale === 'ar' ? 'دورية الدفع' : 'Pay frequency'}</span><strong>{compensation.pay_frequency || '—'}</strong></div></div></section> : null}
